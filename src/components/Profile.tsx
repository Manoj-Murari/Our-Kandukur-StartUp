...
const Profile: React.FC = () => {
  ...
  const handleFormChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    // Apply specific validation based on the input's ID
    if (id === 'name') {
        // Allow only letters and spaces, and convert to uppercase
          ...
          <form onSubmit={handleSave} className="space-y-6">
            ...
            <fieldset className="border p-4 rounded-md"><legend className="text-lg font-semibold px-2">Academic Details</legend>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {stream && stream !== 'Other' && <div><label htmlFor="branch" className="block text-sm font-medium text-gray-700">Branch</label><select id="branch" value={formData.branch} onChange={handleFormChange} className="mt-1 w-full p-2 border rounded-md" disabled={!stream}><option value="">Select...</option>{(academicData[qualification]?.[stream] || []).map(b => <option key={b}>{b}</option>)}</select></div>}
