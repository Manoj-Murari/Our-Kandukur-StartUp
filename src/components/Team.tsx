        ...
        {!loading && !error && (
          <div className="space-y-16">
            {roleOrder.map(role => (
              groupedMembers[role] && (
                <div key={role}>
                  <h3 className="text-3xl font-bold text-gray-800 mb-8 border-l-4 border-blue-600 pl-4">{role}</h3>
                  {/* FIXED: Changed to a responsive grid layout */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
                    {groupedMembers[role].map((member) => (
                      <div key={member.id} className="text-center bg-white p-6 rounded-lg shadow-lg hover:shadow-2xl transform hover:-translate-y-2 transition-all duration-300">
                        <img
                        ...
                        <a href={member.socialLink} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 transition-colors mt-2 inline-block" aria-label={`LinkedIn profile of ${member.name}`}>
